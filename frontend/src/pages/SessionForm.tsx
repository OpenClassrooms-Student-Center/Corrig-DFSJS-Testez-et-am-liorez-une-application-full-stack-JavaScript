...
import { Teacher, SessionFormData, ApiError } from '../types';
...
function SessionForm() {
  ...
  const fetchTeachers = async (signal?: AbortSignal): Promise<void> => {
    try {
      const data = await teacherService.getAll(signal);
      setTeachers(data);
    } catch (err) {
  ...
  const fetchSession = async (signal?: AbortSignal): Promise<void> => {
    try {
      const session = await sessionService.getById(Number(id), signal);
      setFormData({
        name: session.name,
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      if (isEditMode) {
        await sessionService.update(Number(id), formData);
      } else {
